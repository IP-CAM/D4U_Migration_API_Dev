...
class XLSXWriter
{
    ...
    //DIGITAL4U  Add parameter $row_number in order to insert value in specific row
    public function writeSheetHeader($sheet_name, array $header_types, $row_number, $col_options = null)
    {
        //DIGITAL4U Removed !empty($this->sheets[$sheet_name])
        //because sheet starts with writeSheetRow
        ...
        if (!$suppress_row)
        {
            ...
            //DIGITAL4U  Add parameter $row_number in order to insert value in specific row
            $sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="false" hidden="false" ht="12.1" outlineLevel="0" r="' . ($row_number + 1) . '">');
    ...
    //DIGITAL4U  Add parameter $row_number in order to insert value in specific row
    public function writeSheetRow($sheet_name, array $row, $row_number, $row_options=null)
    {
        //DIGITAL4U Removed !empty($this->sheets[$sheet_name])
        //because sheet starts with writeSheetRow
        ...
        $this->initializeSheet($sheet_name, $row_options['widths'] ?? null);
        $sheet = &$this->sheets[$sheet_name];
        ...
        //DIGITAL4U default number
        $row_number = isset($row_number) ? $row_number : $sheet->row_count;
        ...
        if (!empty($row_options))
        {
            ...
            //DIGITAL4U Add parameter $row_number in order to insert value in specific row
            $sheet->file_writer->write('<row collapsed="'.($collapsed ? 'true' : 'false').'" customFormat="false" customHeight="'.($customHt ? 'true' : 'false').'" hidden="'.($hidden ? 'true' : 'false').'" ht="'.($ht).'" outlineLevel="0" r="' . ($row_number + 1) . '" width="20">');
        }
        else
        {
            //DIGITAL4U Add parameter $row_number in order to insert value in specific row
    ...
    public function markMergedCell($sheet_name, $start_cell_row, $start_cell_column, $end_cell_row, $end_cell_column)
    {
        ...
        //DIGITAL4U default $start_cell_row and $end_cell_row
        $start_cell_row = isset($start_cell_row) ? $start_cell_row : $sheet->row_count - 1;
        $end_cell_row = isset($end_cell_row) ? $end_cell_row : $sheet->row_count - 1;
        if ($start_cell_row == $sheet->row_count || $end_cell_row == $sheet->row_count)
        {
            $sheet->row_count++;
        }
